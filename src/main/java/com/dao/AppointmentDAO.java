...
public class AppointmentDAO {  
	...
	public boolean addAppointment(Appointment ap) {
		...
		try {
			...
			String sql = "insert into appointment(user_id,full_name,gender,age,app_date,email,phone_no,diseases,doctor_id,address,status) values(?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean updateCommentStatus(int id, int doctId, String comm) {
		boolean f = false;
		try {
			String sql = "update appointment set status=? where a_id=? and doctor_id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, comm);
			ps.setInt(2, id);
			ps.setInt(3, doctId);
	...
	public List<Appointment> getAllAppointment() {
		...
		try {
			...
			String sql = "select * from appointment order by a_id desc";
			PreparedStatement ps = conn.prepareStatement(sql);
